class CloneMixerQuestionSet < ActiveRecord::Migration
  def up
    answer_map  = {}

    original    = QuestionSet.where(name: 'mixer').first

    replacement = QuestionSet.new(attrs(original))
    replacement.name = 'questions'
  ...
  def down
    replacement = QuestionSet.where(name: 'questions').first
    replacement.questions.each(&:destroy!)
    replacement.dashboard_items.each(&:destroy!)
    replacement.destroy!
  end
