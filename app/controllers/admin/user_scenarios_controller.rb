class Admin::UserScenariosController < AdminController
  ...
  def create
    ...
    if @user_scenario.save
      redirect_to(admin_user_scenario_path(@user_scenario), :notice => 'User Scenario was successfully created.')
    else
      render :action => "new"
    end
  end
  ...
  def update
    if @user_scenario.update_attributes(params[:user_scenario])
      redirect_to(admin_user_scenario_path(@user_scenario), :notice => 'User Scenario was successfully updated.')
    else
      render :action => "edit"
    end
  end
  ...
  def destroy
    ...
    redirect_to(admin_user_scenarios_url, :notice => 'User Scenario deleted')
  end
